#include <stdint.h> // uint8_t
#include <stdio.h>  // printf(), fprintf()
#include <stdlib.h> // size_t, EXIT_SUCCESS, EXIT_FAILURE

#include "aes_lib_ecb_reference.h" // add_round_key(), sub_bytes(), inv_sub_bytes(), shift_row(), inv_shift_row(), mix_columns(), inv_mix_columns()



// public functions
int aes256_ecb_encrypt(uint8_t *data, size_t dataSize, const uint8_t *key, size_t keySize);
int aes256_ecb_decrypt(uint8_t *data, size_t dataSize, const uint8_t *key, size_t keySize);



// private functions
void key256_expansion(const uint8_t *key, uint8_t *expandedKey);



/* Encrypts 'data' with 'key' according to AES256 in ECB mode
 *
 * const uint8_t *data: arbitrary byte-array
 * size_t dataSize:     size of 'data' in byte
 * const uint8_t *key:   arbitrary byte-array of size 32 bytes
 * size_t keySize:       size of 'key' in byte
 */
int aes256_ecb_encrypt(uint8_t *data, size_t dataSize, const uint8_t *key, size_t keySize)
{
	if (keySize != 32 || dataSize % 16 != 0 || !data || !key) {
		fprintf(stderr, "Error: aes256_ecb_encrypt(...) was called with wrong parameters\n");

		return EXIT_FAILURE;
	}

	uint8_t expandedKey[240]; // 240 = 16 * 15 = blocksize * number of add_round_key()


	// Expand key
	key256_expansion(key, expandedKey);

	// Execute algorithm on every 16 bytes of the data
	for (int dataOffset = 0; dataOffset < dataSize; dataOffset += 16) {
		// Initial round
		add_round_key(&data[dataOffset], expandedKey, 0);

		// 13 repeating cycle rounds
		for (int round = 1; round <= 13; round++) {
			sub_bytes(&data[dataOffset]);
			shift_row(&data[dataOffset]);
			mix_columns(&data[dataOffset]);
			add_round_key(&data[dataOffset], expandedKey, round);
		}

		// Final round
		sub_bytes(&data[dataOffset]);
		shift_row(&data[dataOffset]);
		add_round_key(&data[dataOffset], expandedKey, 14);
	}

	return EXIT_SUCCESS;
}



/* Decrypts 'data' with 'key' according to AES256 in ECB mode
 *
 * const uint8_t *data: arbitrary byte-array
 * size_t dataSize:     size of 'data' in byte
 * const uint8_t *key:   arbitrary byte-array of size 32 bytes
 * size_t keySize:       size of 'key' in byte
 */
int aes256_ecb_decrypt(uint8_t *data, size_t dataSize, const uint8_t *key, size_t keySize)
{
	if (keySize != 32 || dataSize % 16 != 0 || !data || !key) {
		fprintf(stderr, "Error: aes256_ecb_decrypt(...) was called with wrong parameters\n");

		return EXIT_FAILURE;
	}

	uint8_t expandedKey[240]; // 240 = 16 * 15 = blocksize * number of add_round_key()


	// Expand key
	key256_expansion(key, expandedKey);

	// Execute algorithm on every 16 bytes of the data
	for (int dataOffset = 0; dataOffset < dataSize; dataOffset += 16) {
		// Initial round
		add_round_key(&data[dataOffset], expandedKey, 14);

		// 13 repeating cycle rounds
		for (int round = 13; round >= 1; round--) {
			inv_shift_row(&data[dataOffset]);
			inv_sub_bytes(&data[dataOffset]);
			add_round_key(&data[dataOffset], expandedKey, round);
			inv_mix_columns(&data[dataOffset]);
		}

		// Final round
		inv_shift_row(&data[dataOffset]);
		inv_sub_bytes(&data[dataOffset]);
		add_round_key(&data[dataOffset], expandedKey, 0);
	}

	return EXIT_SUCCESS;
}



/* Expands the key into the 240 byte expandedKey according to AES specifications
 *
 * const uint8_t *key:   byte-array with initial 32 bytes key
 * uint8_t *expandedKey: arbitrary byte-array of size 240 bytes
 */
void key256_expansion(const uint8_t *key, uint8_t *expandedKey)
{
	static const uint8_t sbox[] = {};
	static const uint8_t rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40};

	// First copy initial key to expandedKey as a basis to expand upon
	for (int round = 0; round < 8; round++) {
		expandedKey[(4 * round) + 0] = key[(4 * round) + 0];
		expandedKey[(4 * round) + 1] = key[(4 * round) + 1];
		expandedKey[(4 * round) + 2] = key[(4 * round) + 2];
		expandedKey[(4 * round) + 3] = key[(4 * round) + 3];
	}

	// Expand the expandedKey according to AES specification. Each round
	// determines the operation on the 4 consecutive bytes at index (4 * round)
	for (int round = 8; round < 60; round++) {
		if (round % 4 == 0) {
			if (round % 8 == 0) {
				// expandedKey[round * 4] = sub_word(rot_word(expandedKey[(round - 1) * 4])) XOR rcon_word((round / 8) - 1) XOR expandedKey[(round - 8) * 4]
				expandedKey[(round * 4) + 0] = sbox[expandedKey[((round - 1) * 4) + 1]] ^ rcon[(round / 8) - 1] ^ expandedKey[((round - 8) * 4) + 0];
				expandedKey[(round * 4) + 1] = sbox[expandedKey[((round - 1) * 4) + 2]] ^ 0x00 ^ expandedKey[((round - 8) * 4) + 1];
				expandedKey[(round * 4) + 2] = sbox[expandedKey[((round - 1) * 4) + 3]] ^ 0x00 ^ expandedKey[((round - 8) * 4) + 2];
				expandedKey[(round * 4) + 3] = sbox[expandedKey[((round - 1) * 4) + 0]] ^ 0x00 ^ expandedKey[((round - 8) * 4) + 3];
			} else {
				// expandedKey[round * 4] = sub_word(expandedKey[(round -1) * 4]) XOR expandedKey[(round - 8) * 4]
				expandedKey[(round * 4) + 0] = sbox[expandedKey[((round - 1) * 4) + 0]] ^ expandedKey[((round - 8) * 4) + 0];
				expandedKey[(round * 4) + 1] = sbox[expandedKey[((round - 1) * 4) + 1]] ^ expandedKey[((round - 8) * 4) + 1];
				expandedKey[(round * 4) + 2] = sbox[expandedKey[((round - 1) * 4) + 2]] ^ expandedKey[((round - 8) * 4) + 2];
				expandedKey[(round * 4) + 3] = sbox[expandedKey[((round - 1) * 4) + 3]] ^ expandedKey[((round - 8) * 4) + 3];
			}
		} else {
			// expandedKey[round * 4] = expandedKey[(round - 1) * 4] XOR expandedKey[(round - 8) * 4]
			expandedKey[(round * 4) + 0] = expandedKey[((round - 1) * 4) + 0] ^ expandedKey[((round - 8) * 4) + 0];
			expandedKey[(round * 4) + 1] = expandedKey[((round - 1) * 4) + 1] ^ expandedKey[((round - 8) * 4) + 1];
			expandedKey[(round * 4) + 2] = expandedKey[((round - 1) * 4) + 2] ^ expandedKey[((round - 8) * 4) + 2];
			expandedKey[(round * 4) + 3] = expandedKey[((round - 1) * 4) + 3] ^ expandedKey[((round - 8) * 4) + 3];
		}
	}
}

