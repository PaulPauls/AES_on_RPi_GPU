...
#include "aes256_ecb_reference.h" // aes128_ecb_encrypt()
#include "aes256_ecb_gpu.h" // aes128_ecb_gpu_encrypt()
#include "aes256_ecb_gpu_prePipe.h"
#include "rijndael.h"
...
int main(int argc, char **argv)
{
  ...
  for (int nor = 0; nor < NUM_RUNS; nor++) {
    ...
    if (ENCRYPT_ELSE_DECRYPT) {
      ...
      clock_gettime(CLOCK_REALTIME, &start);
      unsigned long expandedKey[60];
      rijndaelSetupEncrypt(expandedKey, key, 256);
      for (int i = 0; i < (sizeof(data1) / sizeof(data1[0])); i = i+16) {
        rijndaelEncrypt(expandedKey, 14, data4_in + i, data4_out + i);
      }
      clock_gettime(CLOCK_REALTIME, &stop);
      timeDiff = (stop.tv_sec - start.tv_sec)
               + (stop.tv_nsec - start.tv_nsec) / 1E3;
      ...
      if (PRELOAD_PIPE) {
        ...
        timeDiff = aes256_ecb_gpu_encrypt_prePipe(data1, sizeof(data1), key);
        timeGPUTotal = timeGPUTotal + (uint64_t)timeDiff;
        write_to_csv(csvFP, (uint64_t) timeDiff, 0);
      } else {
        // GPU Execution of AES without pre loaded Pipe
        clock_gettime(CLOCK_REALTIME, &start);
        aes256_ecb_gpu_encrypt(data1, sizeof(data1), key);
        clock_gettime(CLOCK_REALTIME, &stop);
        timeDiff = (stop.tv_sec - start.tv_sec)
                 + (stop.tv_nsec - start.tv_nsec) / 1E3;
        timeGPUTotal = timeGPUTotal + (uint64_t)timeDiff;
      ...
      clock_gettime(CLOCK_REALTIME, &start);
      aes256_ecb_encrypt(data2, sizeof(data2), key, sizeof(key));
      clock_gettime(CLOCK_REALTIME, &stop);
      timeDiff = (stop.tv_sec - start.tv_sec)
               + (stop.tv_nsec - start.tv_nsec) / 1E3;
      ...
      clock_gettime(CLOCK_REALTIME, &start);
      unsigned long expandedKey[60];
      rijndaelSetupDecrypt(expandedKey, key, 256);
      for (int i = 0; i < (sizeof(data1) / sizeof(data1[0])); i = i+16) {
        rijndaelDecrypt(expandedKey, 14, data4_in + i, data4_out + i);
      }
      clock_gettime(CLOCK_REALTIME, &stop);
      timeDiff = (stop.tv_sec - start.tv_sec)
               + (stop.tv_nsec - start.tv_nsec) / 1E3;
      ...
      if (PRELOAD_PIPE) {
        // GPU Execution of AES with pre loaded Pipe
        timeDiff = aes256_ecb_gpu_decrypt_prePipe(data1, sizeof(data1), key);
        timeGPUTotal = timeGPUTotal + (uint64_t)timeDiff;
        write_to_csv(csvFP, (uint64_t) timeDiff, 0);
      } else {
        // GPU Execution of AES without pre loaded Pipe
        clock_gettime(CLOCK_REALTIME, &start);
        aes256_ecb_gpu_decrypt(data1, sizeof(data1), key);
        clock_gettime(CLOCK_REALTIME, &stop);
        timeDiff = (stop.tv_sec - start.tv_sec)
                 + (stop.tv_nsec - start.tv_nsec) / 1E3;
        timeGPUTotal = timeGPUTotal + (uint64_t)timeDiff;
      ...
      // CPU Execution of AES Reference
      clock_gettime(CLOCK_REALTIME, &start);
      aes256_ecb_decrypt(data2, sizeof(data2), key, sizeof(key));
      clock_gettime(CLOCK_REALTIME, &stop);
      timeDiff = (stop.tv_sec - start.tv_sec)
               + (stop.tv_nsec - start.tv_nsec) / 1E3;
      timeReferenceTotal = timeReferenceTotal + (uint64_t)timeDiff;
