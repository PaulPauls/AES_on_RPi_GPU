...
#define QPU_ENC_CODEFILE "./aes256_ecb_gpu/aes256_ecb_gpu_encrypt.bin"
#define QPU_DEC_CODEFILE "./aes256_ecb_gpu/aes256_ecb_gpu_decrypt.bin"
#define NUM_QPUS        (12)
...
#define KEY_WORDSIZE    (60)  // 60 words for 240 byte Key
...
void load_qpu_code_prePipe(const char *qpuCodeFile, uint32_t *qpuCode, uint32_t bytecount);
...
/* Encrypts 'text' with 'key' according to AES256 in ECB mode on the RPi GPU.
...
double aes256_ecb_gpu_encrypt_prePipe(uint8_t *text, const size_t textBytecount, const uint8_t *key)
{
  ...
  // Load the QPU code
  load_qpu_code_prePipe(QPU_ENC_CODEFILE, localQPUCode, QPU_ENC_CODE_BYTECOUNT);
  ...
  AES_set_encrypt_key(key, 256, &expandedKey);
...
/* Decrypts 'text' with 'key' according to AES256 in ECB mode on the RPi GPU.
...
double aes256_ecb_gpu_decrypt_prePipe(uint8_t *text, const size_t textBytecount, const uint8_t *key)
{
  ...
  // Load the QPU code
  load_qpu_code_prePipe(QPU_DEC_CODEFILE, localQPUCode, QPU_DEC_CODE_BYTECOUNT);
